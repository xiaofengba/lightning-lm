...
bool LocSystem::Init(const std::string &yaml_path) {
    ...
    init_pose_sub = node_->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
        "/initialpose",         // 订阅的话题名（可根据实际需求修改）
        10,                      // 消息队列大小（避免消息丢失）
        [this](geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg) {
            auto position = msg->pose.pose.position;  // 位置 x/y/z
            auto orientation = msg->pose.pose.orientation;  // 姿态四元数
            Eigen::Vector3d t_initial(position.x, position.y,  position.z);
            SetInitPose(SE3::trans(t_initial));
        }
    );

...
bool LidarLoc::UpdateGlobalMap() {
    ...
    if (!loc_inited_) {
        ...
        ndt_rough->setMaximumIterations(10);
...
//     // 改进，这里不能直接使用lo的pose，而是需要计算一个lopose的初值，然后后续叠加上去
//     // SE3 guess_from_lo = current_lo_pose_;
...
//     // 如果定位成功了就更新， last_abs_pose, 也就是重定位的位置
//     if (loc_success) {

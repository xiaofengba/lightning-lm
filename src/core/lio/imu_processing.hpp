...
class ImuProcess {
   public:
    ...
    void estimate_so3_no_yaw(const Vec3d& mean_acc_, NavState& state, double g = 9.81);
...
inline void ImuProcess::IMUInit(const MeasureGroup &meas, ESKF &kf_state, int &N) {
    ...
    // init_state.grav_ = S2(-Vec3d(0, 0, 1) * G_m_s2);
    std::cout << "===========================================" << std::endl;
    // std::cout << init_state.grav_ << std::endl; init_state.rot_ = 
    std::cout << mean_acc_ << std::endl;
    estimate_so3_no_yaw(mean_acc_, init_state,  G_m_s2);

    
    init_state.bg_ = mean_gyr_;
    init_state.offset_t_lidar_ = t_lidar_mu_;
    init_state.offset_R_lidar_ = R_lidar_imu_;
    // acc with g to rot: c3d pos_ SO3 rot_
...
inline void ImuProcess::estimate_so3_no_yaw(const Vec3d& mean_acc_, NavState& state, double g) {
    // 1. 归一化 IMU 系重力向量（去除缩放误差）
    Vec3d g_b = mean_acc_;
    double g_b_norm = g_b.norm();
    if (g_b_norm < 1e-6) {
        throw std::runtime_error("mean_acc_ 接近零，无法估计姿态");
    }
    g_b.normalize(); // 得到单位重力向量 z_b

    // 2. 构造 IMU 系 x 轴（世界系 x 轴投影，yaw=0）
    Vec3d x_w(1.0, 0.0, 0.0); // 世界系 x 轴
    Vec3d x_b = x_w - (g_b.dot(x_w)) * g_b; // 投影到 IMU 水平平面（垂直于 z_b）
    x_b.normalize(); // 归一化，确保单位向量

    // 3. 构造 IMU 系 y 轴（右手系正交）
    Vec3d y_b = g_b.cross(x_b); // z_b × x_b = y_b（保证正交）
    y_b.normalize();

    // 4. 构造 SO(3) 旋转矩阵（列向量是 IMU 系基向量，转置后为 IMU->世界系旋转）
    Mat3d R;
    R.col(0) = x_b;
    R.col(1) = y_b;
    R.col(2) = g_b;
    // R = R.transpose(); // 关键：SO(3) 旋转矩阵的逆等于转置，此处转置后得到 IMU->世界系
    R.transposeInPlace(); 

    // 验证：确保 R 是正交矩阵（SO(3) 约束）
    assert(std::abs(R.determinant() - 1.0) < 1e-6);
    // return R;

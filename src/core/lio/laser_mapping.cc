...
bool LaserMapping::LoadParamsFromYAML(const std::string &yaml_file) {
    ...
    try {
        ...
        // global map
        global_map_filter_in_ego_z_min = yaml["map_save"]["global_map_filter_in_ego_z_min"].as<float>();
        global_map_filter_in_ego_z_max = yaml["map_save"]["global_map_filter_in_ego_z_max"].as<float>();
...
bool LaserMapping::Run() {
    ...
    if (last_kf_ == nullptr) {
        ...
        if (((last_pose.translation() - cur_pose.translation()).norm() > options_.kf_dis_th_ ||
            ...
            // &&(state_point_.timestamp_ - last_kf_->GetState().timestamp_) > 1.5
        ) {
            MakeKF();
...
CloudPtr LaserMapping::GetGlobalMap(bool use_lio_pose, bool use_voxel, float res) {
    ...
    double ground_min =global_map_filter_in_ego_z_min, ground_max = global_map_filter_in_ego_z_max;
    Vec3d  cur_translation;
    ...
    for (auto &kf : all_keyframes_) {
        ...
        if (use_lio_pose) {
            cur_translation = kf->GetLIOPose().translation();
            pcl::transformPointCloud(*cloud_filter, *cloud_trans, kf->GetLIOPose().matrix());
        } else {
            cur_translation = kf->GetOptPose().translation();
            pcl::transformPointCloud(*cloud_filter, *cloud_trans, kf->GetOptPose().matrix());
        }

        // 1124 tangjialiang ★★★ 高度滤除 ★★★
        CloudPtr cloud_filtered_height(new PointCloudType);
        cloud_filtered_height->reserve(cloud_trans->size());

        for (auto &pt : cloud_trans->points) {
            if (pt.z >= (cur_translation.z()+ground_max) || pt.z < (cur_translation.z()+ground_min)) {     // <- 保留高于 1.5m
                cloud_filtered_height->push_back(pt);
            }
        }
        cloud_trans = cloud_filtered_height;
